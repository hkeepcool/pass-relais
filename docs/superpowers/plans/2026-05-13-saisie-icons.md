# Saisie Icons Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace abstract unicode glyphs on the Saisie (PatientDetailPage) quick-tap buttons with Lucide SVG icons for consistent, semantically clear visuals.

**Architecture:** Two tasks in order — first widen `QuickTapButton.glyph` from `string` to `ReactNode` (one-line type change, verified by TypeScript), then replace every glyph value in `PatientDetailPage` with the matching Lucide icon component or inline SVG. No logic changes anywhere.

**Tech Stack:** React 18, TypeScript, Vitest + jsdom + @testing-library/react, lucide-react
...
|--------|------|----------------|
| Modify | `src/design-system/components/QuickTapButton.tsx` | Widen `glyph` type: `string` → `ReactNode` |
| Modify | `src/features/patients/PatientDetailPage.tsx` | Replace all glyph values with Lucide icons + inline dot SVGs |
| Add dep | `package.json` / `node_modules` | `lucide-react` |

---

## Task 1: Widen QuickTapButton.glyph to ReactNode

**Files:**
- Modify: `src/design-system/components/QuickTapButton.tsx`

- [ ] **Step 1: Verify current TypeScript rejects a JSX glyph**

Run:
```
npx tsc -b --noEmit 2>&1 | head -5
```
Expected: only the two pre-existing errors (`flush.ts:30`, `vite.config.ts:47`). This confirms the baseline before we change anything.

- [ ] **Step 2: Change the glyph type**

In `src/design-system/components/QuickTapButton.tsx`, change line 3–4:
...
No other changes in this file — the render already uses `{glyph}` which handles both strings and ReactNodes identically.

- [ ] **Step 3: Run TypeScript to confirm no new errors**

```
npx tsc -b --noEmit
```
Expected: still only the two pre-existing errors (`flush.ts:30`, `vite.config.ts:47`). Zero new errors.
...
```
git add src/design-system/components/QuickTapButton.tsx
git commit -m "feat: widen QuickTapButton glyph type to ReactNode"
```

---

## Task 2: Replace glyphs in PatientDetailPage
...
- [ ] **Step 2: Replace the full PatientDetailPage.tsx**

Replace the file content with the following. The only changes from the current file are: the added Lucide import, the updated option array types, and the new glyph values. All state, handlers, JSX layout, and submit logic are unchanged.
...
```
git add src/features/patients/PatientDetailPage.tsx package.json package-lock.json
git commit -m "feat: replace unicode glyphs with Lucide SVG icons on Saisie page"
```
...
**Spec coverage:**
- ✅ Add `lucide-react` — Task 2 Step 1
- ✅ `QuickTapButton.glyph`: `string` → `ReactNode` — Task 1 Step 2
- ✅ Sommeil: Moon / Waves / EyeOff — Task 2 Step 2
- ✅ Appétit: Utensils / MinusCircle / XCircle — Task 2 Step 2
- ✅ Douleur: Smile / Meh / Frown — Task 2 Step 2
- ✅ Humeur: Check / HelpCircle / Activity — Task 2 Step 2
- ✅ Selles: inline dot SVGs — Task 2 Step 2 (`DOT_SVG` helper)
- ✅ Icon size 22, strokeWidth 1.75 — `ICON_SIZE` / `ICON_STROKE` constants

**Placeholder scan:** No TBD/TODO. All code blocks are complete and self-contained.

**Type consistency:**
- `ReactNode` imported from `'react'` in both `QuickTapButton.tsx` (already imported) and `PatientDetailPage.tsx` (added) ✓
- `SLEEP_OPTIONS` tuple type `[SleepValue, string, ReactNode, TapTone][]` matches destructuring `[val, label, glyph, tone]` ✓
- `BOWEL_OPTIONS` tuple type `[BowelMovementsValue, string, ReactNode][]` matches destructuring `[val, label, glyph]` ✓
- `DOT_SVG` returns `ReactNode` — matches `BOWEL_OPTIONS` third element type ✓
- `QuickTapButton glyph` prop accepts `ReactNode` after Task 1 — used as `ReactNode` in Task 2 ✓
